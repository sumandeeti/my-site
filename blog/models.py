...
class Post(models.Model):  #defines the model( it is an object)
							#models.Model means that the Post is a Django Model, so Django knows that is should be saved in the database.
	author = models.ForeignKey('auth.User', on_delete=models.CASCADE) #this is a link to another model.
	title = models.CharField(max_length=200) #define text with a limited number of charcters.
